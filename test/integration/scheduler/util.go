...
func initTestScheduler(
	...
	// Initialize a new context so we can create multiple schedulers by this function.
	var context TestContext
	...
	context.schedulerConfigFactory = createConfiguratorWithPodInformer(
		schedulerName, context.clientSet, podInformer, context.informerFactory)
...
func initTest(t *testing.T, nsPrefix string) *TestContext {
	return initTestScheduler(v1.DefaultSchedulerName, t, initTestMaster(t, nsPrefix, nil), nil, true, nil)
}
