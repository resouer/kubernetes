...
func TestCloseWatchChannelOnError(t *testing.T) {
	...
	go r.ListAndWatch(utiltime.NeverStop)
	fw.Error(pod)
	select {
	...
	case <-time.After(utiltime.ForeverTestTimeout):
		t.Errorf("the cancellation is at least %s late", utiltime.ForeverTestTimeout.String())
		break
	}
}
...
func TestReflectorResyncChan(t *testing.T) {
	...
	a, _ := g.resyncChan()
	b := time.After(utiltime.ForeverTestTimeout)
	select {
	case <-a:
...
func TestReflectorWatchHandlerError(t *testing.T) {
	...
	var resumeRV string
	err := g.watchHandler(fw, &resumeRV, neverExitWatch, utiltime.NeverStop)
	if err == nil {
...
func TestReflectorWatchHandlerTimeout(t *testing.T) {
	...
	exit <- time.Now()
	err := g.watchHandler(fw, &resumeRV, exit, utiltime.NeverStop)
...
func TestReflectorListAndWatchWithErrors(t *testing.T) {
	...
	for line, item := range table {
		...
		r := NewReflector(lw, &api.Pod{}, s, 0)
		r.ListAndWatch(utiltime.NeverStop)
	}
}
