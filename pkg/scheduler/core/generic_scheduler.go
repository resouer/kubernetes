...
func (g *genericScheduler) filterNodeToVictimsByExtenders(
	...
	for _, extender := range g.extenders {
		...
		filteredNodes, failedNodesMap, err := extender.Filter(pod, candidateNodes, nodeNameToInfoCopy)
		if err != nil {
		...
		if len(filteredNodes) == 0 {
			// No candidate node can run the preemptor pod, abort the preemption by returning a
			// empty nodeToVictims.
		...
		for node, _ := range nodeToVictims {
			if _, found := failedNodesMap[node.GetName()]; found {
